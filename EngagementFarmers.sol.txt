...
contract EngagementFarmers is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 private WHITELIST_LIMIT = 2;
    uint256 private PUBLIC_LIMIT = 1;
    uint256 public constant MINT_FEES = 0.08 ether;
    ...
    constructor() ERC721A("Engagement Farmers", "$FRMR") {
    ...
    function whitelistMint(uint256 quantity, bytes32[] memory proof) external payable nonReentrant {
        require(whenWhitelistSaleIsOn() == true, "EF: Whitelist Sale Not Started Yet");
        ...
        require(whitelistCounter[msg.sender] <= WHITELIST_LIMIT, "EF: Whitelist Limit Exceeded");
        whitelistMinted+=quantity;
        require(whitelistMinted <= MAX_WHITELIST_LIMIT, "EF: Max Whitelist Limit Exceeded");
        require(msg.value == quantity.mul(MINT_FEES), "EF: Send proper mint fees");
        payable(owner()).transfer(msg.value);
        require(totalSupply().add(quantity) <= MAX_SUPPLY, "EF: Mint would exceed max limit");
        _safeMint(msg.sender, quantity);
    }

    function publicMint(uint256 quantity, bytes calldata signature) external payable nonReentrant {
        require(whenPublicSaleIsOn() == true, "EF: Public Sale Not Started Yet");
        require(usedSign[signature] == false, "EF: Signature is used");
        ...
        require(publicCounter[msg.sender] <= PUBLIC_LIMIT, "EF: Public Limit Exceeded");
        require(msg.value == quantity.mul(MINT_FEES), "EF: Send proper mint fees");
        payable(owner()).transfer(msg.value);
        require(totalSupply().add(quantity) <= MAX_SUPPLY, "EF: Mint would exceed max limit");
        _safeMint(msg.sender, quantity);
    ...
    function ownerMint(uint256 quantity) public onlyOwner {
        require(totalSupply().add(quantity) <= MAX_SUPPLY, "EF: Exceeding Max Limit");
        _safeMint(msg.sender, quantity);
    }

    function burn(uint256 tokenId) public {
        require(ownerOf(tokenId) == msg.sender, "EF: You are not Owner");
        _burn(tokenId);
    }
